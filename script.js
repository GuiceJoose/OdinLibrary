...
submit.addEventListener('click', makeNewBook)
...
function updateLibrary() {
    ...
    readButtons.forEach((button)=>button.addEventListener('click', readFlipper))
...
function removeBook(e) {
    let thisBook = e.target.parentNode.firstChild.textContent.replaceAll('"','');
    myLibrary.splice(myLibrary.findIndex(object => object.title == thisBook), 1)
    updateLibrary()
}
...
function makeNewBook() {
    ...
    closeBookModal();
    let newBook = new aBook(newTitle.value, newAuthor.value, newPages.value, convertToRead(newRead.checked));
    addBook(newBook);
    updateLibrary();
}
